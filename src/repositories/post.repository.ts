...
export const postRepository = {
  ...
  async getPostMeta(id: number) {
    return prisma.post.findUnique({
      where: { id },
      select: { id: true, title: true, slug: true, command: true, status: true, isPublished: true, editNavigation: true },
    });
  },
  ...
  async getPublished() {
    return prisma.post.findMany({
      ...
      include: { messages: { orderBy: { order: 'asc' } }, commands: true, richMedia: { orderBy: { order: 'asc' } }, richEntities: true, keyboards: { orderBy: [{ row: 'asc' }, { col: 'asc' }] }, _count: { select: { views: true } } },
      orderBy: [{ sortOrder: 'asc' }, { updatedAt: 'desc' }],
    });
  },
  ...
  async getDrafts() {
    return prisma.post.findMany({
      ...
      include: { messages: { orderBy: { order: 'asc' } }, _count: { select: { views: true } } },
  ...
  async getTopPosts(limit: number = 5) {
    return prisma.post.findMany({
      where: { status: PostStatus.PUBLISHED, isPublished: true },
      include: { messages: { orderBy: { order: 'asc' } }, _count: { select: { views: true, clickLogs: true } } },
  ...
  async restoreVersion(versionId: number) {
    ...
    const post = await prisma.post.update({
      ...
      data: {
        ...
        contentEntities: snapshot.contentEntities ?? null,
        renderMode: snapshot.renderMode ?? 'telegram_entities',
        previewText: snapshot.previewText ?? null,
        telegramPayload: snapshot.telegramPayload ?? Prisma.DbNull,
        telegramMessageSnapshot: snapshot.telegramMessageSnapshot ?? Prisma.DbNull,
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const post = await tx.post.create({
        data: {
          ...
          contentEntities: original.contentEntities ?? undefined,
          renderMode: original.renderMode ?? 'telegram_entities',
          previewText: original.previewText,
          telegramPayload: original.telegramPayload ?? undefined,
          telegramMessageSnapshot: original.telegramMessageSnapshot ?? undefined,
