...
function formatDiscount(d: any, index?: number): string {
  ...
  return (
    `${prefix}🏢 *${d.propFirm.name}*\n` +
    `📌 ${d.title}\n` +
    `💸 تخفیف: *${d.discountPercent}%*\n` +
    `🔑 کد: \`${d.code}\`` +
    expired +
    `\n👆 استفاده: ${d.usageCount} بار`
  );
}
...
export function registerHandlers(bot: Telegraf) {
  ...
  bot.action(/^cat:(.+)$/, async (ctx) => {
    ...
    const result =
      category === 'ALL'
        ...
        : await discountService.getByCategory(category as DiscountCategory, page);
  ...
  bot.action(/^copy:(\d+)$/, async (ctx) => {
    ...
    if (!discount) return ctx.answerCbQuery('کد یافت نشد');
  ...
  bot.hears('🏢 پراپ فرم‌ها', async (ctx) => {
    const firms = await discountService.getPropFirms();
    if (firms.length === 0) {
      return ctx.reply('❌ هنوز پراپ فرمی ثبت نشده.');
    }
    const text = firms
      .map((f: any) => `🏢 *${f.name}* — ${f._count.discountCodes} کد تخفیف`)
      .join('\n');
  ...
  bot.hears('🎰 قرعه‌کشی', async (ctx) => {
    ...
    const userId = (await userService.getProfile(BigInt(ctx.from.id)))?.id;
    const hasEntered = userId
      ? await lotteryService.enterLottery(BigInt(ctx.from.id), lottery.id).then(() => false).catch(() => true)
      : false;

    const endDate = new Date(lottery.endAt).toLocaleDateString('fa-IR');
    await ctx.reply(
      `🎰 *${lottery.title}*\n\n` +
        `🏆 جایزه: ${lottery.prize}\n` +
        `👥 شرکت‌کنندگان: ${lottery._count.entries} نفر\n` +
  ...
  bot.action(/^lottery:enter:(\d+)$/, async (ctx) => {
    await ctx.answerCbQuery();
  ...
  bot.hears('⭐️ امتیاز من', async (ctx) => {
    ...
    if (!profile) return ctx.reply('❌ خطا در دریافت اطلاعات');
  ...
  bot.hears('🏆 لیدربورد', async (ctx) => {
    ...
    const medals = ['🥇', '🥈', '🥉'];
    const text = board
      .map((u: any, i: number) => `${medals[i] || `${i + 1}.`} ${u.firstName} — ${u.points} امتیاز`)
      .join('\n');
