...
interface Props { initial?: DiscountCode; loading?: boolean; submitLabel?: string; onSubmit: (payload: DiscountPayload) => void; }
...
export default function DiscountForm({ initial, loading, submitLabel = "ذخیره", onSubmit }: Props) {
  ...
  return <form className="grid gap-4 md:grid-cols-2" onSubmit={handleSubmit((values) => onSubmit({ ...values, code: values.code.toUpperCase(), affiliateLink: values.affiliateLink || null, expiresAt: safeToISOString(values.expiresAt) }))}>
