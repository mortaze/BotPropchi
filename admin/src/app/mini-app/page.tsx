...
type AppDataResponse = { success: boolean; settings: MiniAppSettings; features: Record<string, boolean>; propFirms: PropFirm[]; error?: string; forceJoinRequired?: boolean; channels?: Array<{ title: string; inviteLink?: string | null; channelId: string; buttonText?: string | null }>; joinButtonText?: string };
...
async function requestDiscounts(initData: string, propFirmId: number) { return jsonFetch<DiscountsResponse>(`${API_BASE_URL}/api/mini-app/prop-firms/${propFirmId}/discounts`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ initData }) }); }
...
export default function TelegramMiniAppPage() {
  ...
  const navItems = useMemo(() => [
    ...
    { key: "firms" as const, label: "پراپ فرم‌ها", icon: Building2, visible: features.prop_firms !== false },
  ...
  ].filter((item) => item.visible), [features.prop_firms, features.referrals, settings.siteUrl]);
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      try {
        ...
        setSettings(appData.settings); setFeatures(appData.features || {}); setPropFirms(appData.propFirms);
  ...
  const openDiscounts = async (firm: PropFirm) => {
    ...
    try { setDiscounts((await requestDiscounts(initData, firm.id)).items); }
