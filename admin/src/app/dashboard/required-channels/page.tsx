...
export default function RequiredChannelsPage() {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><h2 className="font-semibold">لیست عضویت اجباری</h2></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><table className="data-table"><thead><tr><th>عنوان</th><th>شناسه</th><th>نوع</th><th>وضعیت</th><th>عملیات</th></tr></thead><tbody>{query.data?.items.map((item: RequiredChannel) => <tr key={item.id}><td>{item.title}</td><td dir="ltr">{item.chatId || item.channelId}</td><td><Badge variant="info">{item.type === "CHANNEL" ? "کانال" : "گروه"}</Badge></td><td><Toggle checked={item.isActive} onChange={(v) => update.mutate({ id: item.id, payload: { isActive: v } })} /></td><td><button className="text-red-500" onClick={() => remove.mutate(item.id)}><Trash2 className="h-4 w-4" /></button></td></tr>)}</tbody></table></div>{!query.isLoading && !query.data?.items.length && <EmptyState icon={<RadioTower />} title="موردی ثبت نشده" />}</CardContent></Card>
  </div>;
}
